...
Future<void> _installTableTriggers(Database db, String table) async {
  ...
  await db.execute('''
    ...
    BEGIN
      ...
      SELECT '$table', CAST(NEW.$pk AS TEXT), 'insert', COALESCE(NEW.updated_at, strftime('%Y-%m-%dT%H:%M:%fZ','now'))
      WHERE $guard;
    END
  ''');
  ...
  await db.execute('''
    ...
    BEGIN
      ...
      INSERT INTO _sync_outbox(table_name, row_pk, op, changed_at)
      SELECT '$table', CAST(NEW.$pk AS TEXT), 'update', strftime('%Y-%m-%dT%H:%M:%fZ','now')
  ...
  await db.execute('''
    ...
    BEGIN
      INSERT INTO _sync_outbox(table_name, row_pk, op, changed_at)
      SELECT '$table', CAST(OLD.$pk AS TEXT), 'delete', strftime('%Y-%m-%dT%H:%M:%fZ','now')
      WHERE $guard;
    END
  ''');
