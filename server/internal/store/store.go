	...
	rows, err := s.pool.Query(ctx, `
		SELECT * FROM (
		  (SELECT row_pk, data, server_updated_at, updated_at FROM records
		    ...
		    ORDER BY server_updated_at, row_pk LIMIT $5)
		  UNION ALL
...
func (s *Store) HasData(ctx context.Context, companyID string) (bool, error) {
	var ok bool
	err := s.pool.QueryRow(ctx,
		`SELECT EXISTS (SELECT 1 FROM records WHERE company_id = $1) OR EXISTS (SELECT 1 FROM tombstones WHERE company_id = $1)`, companyID).Scan(&ok)
	return ok, err
}
